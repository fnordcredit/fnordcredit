  ...
  app.post('/user/credit', (req, res) => {
    ...
    .then(() => {
        ...
        if (delta < 0 && user.credit + delta < 0) {
          ...
          if (!user.debtAllowed) {
            ...
            if (user.debtHardLimit && user.credit + delta < user.debtHardLimit) {
              ...
              winston.error(
                `[userCredit] credit below ${user.debtHardLimit ||
                  ''} for user ${user.name} not allowed`,
                );
                return;
              }
            }
      ...
      async function getUserAsync(username, cb) {
        const user = await UserModel.where({ name: username })
        .fetch({ columns: ['name', 'lastchanged', 'credit'] });
        if (user) {
      ...
      async function getUserByTokenAsync(token, cb) {
        const user = await UserModel.where({ token })
        .fetch({ columns: ['name', 'lastchanged', 'credit'] });
        if (user) {
          cb(undefined, user.serialize());
        } else {
